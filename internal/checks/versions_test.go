...
func TestKubernetesVersionSupportCheck(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:          "supported minor, far from EOL",
		...
		{
			name:          "minor less than 90 days from EOL",
		...
		{
			name:          "supported minor, outside the warn window",
			server:        &version.Info{Major: "1", Minor: "34", GitVersion: "v1.34.1"},
			now:           base,
			wantStatus:    "pass",
			wantResources: []string{},
		...
		{
			name:          "minor newer than the built-in table",
		...
		{
			name:          "major newer than the built-in table",
		...
		{
			name:          "minor older than the built-in table",
